/**
 * BTC HD-account scan. Structurally identical to src/lib/isk/scan.ts —
...
function hintKey(root: BIP32Interface, kind: AddressKind): string {
  return `hme.btc.scan-hint.${kind}.${root.neutered().toBase58().slice(0, 32)}`;
}
...
export async function scanBtcAccount(
  root: BIP32Interface,
  kind: AddressKind,
  opts?: { deep?: boolean },
  ...
  if (kind === "bip84" || kind === "bip49") {
    ...
    const { BTC_NETWORK } = await import("./network");

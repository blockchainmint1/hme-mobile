/**
 * IskanderCoin address derivation + transaction signing.
 *
 * Reuses the currently-unlocked BIP32 root (Bitcoin xpub bytes are identical),
 * but derives under ISK's SLIP-44 coin type 969696 and encodes addresses with
 * ISK network bytes (K… legacy / isk1q… native segwit).
 */
import * as ecc from "@bitcoinerlab/secp256k1";
import type { BIP32Interface } from "bip32";
import { payments, Psbt } from "bitcoinjs-lib";
import { ISK_NETWORK, ISK_DERIVATION_PATHS, type IskDerivationKind } from "./network";

export type AddressKind = IskDerivationKind;
...
function pathFor(kind: AddressKind, change: 0 | 1, index: number): string {
  return `${ISK_DERIVATION_PATHS[kind]}/${change}/${index}`;
}
...
function deriveAddressFromNode(node: BIP32Interface, kind: AddressKind): string {
  ...
  switch (kind) {
    case "bip84": {
      const p = payments.p2wpkh({ pubkey, network: ISK_NETWORK });
      if (!p.address) throw new Error("Failed to derive isk1q bech32 address");
      return p.address;
    }
    case "bip49": {
      const inner = payments.p2wpkh({ pubkey, network: ISK_NETWORK });
      const p = payments.p2sh({ redeem: inner, network: ISK_NETWORK });
      if (!p.address) throw new Error("Failed to derive isk p2sh address");
      return p.address;
    }
    case "bip44": {
      const p = payments.p2pkh({ pubkey, network: ISK_NETWORK });
      if (!p.address) throw new Error("Failed to derive isk legacy address");
      return p.address;
    }
  }
}
...
export function buildAndSignTx(args: {
  ...
  const psbt = new Psbt({ network: ISK_NETWORK, maximumFeeRate: 10_000_000 });
  ...
  for (const u of inputs) {
    ...
    if (kind === "bip84") {
      ...
      const node = root.derivePath(`${ISK_DERIVATION_PATHS[kind]}/${u.change}/${u.index}`);
      const inner = payments.p2wpkh({ pubkey: node.publicKey, network: ISK_NETWORK });
      base.witnessUtxo = {
        script: hexToBytes(u.witnessScriptHex),
        value: BigInt(u.value),
      };

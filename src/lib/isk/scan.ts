/**
 * ISK HD-account scan. Copy of src/lib/txc/scan.ts pointed at the ISK
 * mempool endpoint and derivation. Shares the BIP44 gap-limit convention
 * (20 consecutive unused addresses per chain) and persists a small hint
 * to keep refreshes cheap after the first deep scan.
 */
...
function hintKey(root: BIP32Interface, kind: AddressKind): string {
  return `hme.isk.scan-hint.${kind}.${root.neutered().toBase58().slice(0, 32)}`;
}
...
export async function scanIskAccount(
  root: BIP32Interface,
  kind: AddressKind,
  opts?: { deep?: boolean },
  ...
  if (kind === "bip84" || kind === "bip49") {
    ...
    const { ISK_NETWORK } = await import("./network");

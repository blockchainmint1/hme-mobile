/**
 * Litecoin USD price via CoinGecko (public, no key). Falls back to CMC if a
...
export const getLtcPriceUsd = createServerFn({ method: "GET" }).handler(
  async (): Promise<PriceQuote> => {
    try {
      ...
      if (res.ok) {
        const json = (await res.json()) as { litecoin?: { usd?: number } };
        const p = json.litecoin?.usd;
    ...
    if (key) {
      try {
        const res = await fetch(
          "https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=LTC&convert=USD",
        ...
        if (res.ok) {
          ...
          const p = json.data?.LTC?.[0]?.quote?.USD?.price;

/**
 * DOGE HD-account scan. Same shape as src/lib/isk/scan.ts — BIP44 legacy
 * only. Uses persisted hint to keep refreshes cheap after the first deep
 * scan.
 */
...
export interface AccountUtxo extends UtxoInput { address: string }
...
function hintKey(root: BIP32Interface, kind: AddressKind): string {
  return `hme.doge.scan-hint.${kind}.${root.neutered().toBase58().slice(0, 32)}`;
}
...
async function scanChain(
  root: BIP32Interface, kind: AddressKind, change: 0 | 1,
): Promise<{ all: DerivedAddress[]; firstUnusedIndex: number }> {
  const all: DerivedAddress[] = [];
  let firstUnused = 0, gap = 0, i = 0;
  while (gap < GAP_LIMIT) {
...
async function scanChainFast(
  root: BIP32Interface, kind: AddressKind, change: 0 | 1, knownUsed: number,
...
export async function scanDogeAccount(
  root: BIP32Interface,
  kind: AddressKind,
  opts?: { deep?: boolean },

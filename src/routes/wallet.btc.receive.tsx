...
import { deriveAddress } from "@/lib/btc/wallet";
...
export const Route = createFileRoute("/wallet/btc/receive")({
  head: () => ({ meta: [{ title: "Receive BTC — HME Wallet" }] }),
  component: ReceiveBtcPage,
});

function ReceiveBtcPage() {
  ...
  const account = useQuery({
    queryKey: ["btc-account", BTC_DEFAULT_KIND, accountId],
  ...
  const shown = useMemo(() => {
    ...
    const derived = deriveAddress(root, BTC_DEFAULT_KIND, 0, idx);
  ...
  return (
    <main className="mx-auto max-w-xl px-4 py-8">
      ...
      <h1 className="mt-3 text-2xl font-bold">Receive BTC</h1>
      ...
      <Card className="mt-6">
        ...
        <CardContent className="flex flex-col items-center gap-4">
          {account.isLoading || !shown ? (
            ...
            <>
              <QrCode value={`bitcoin:${address}`} size={240} />
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                ...
                <Button variant="secondary" onClick={async () => {
                  const ok = await shareText({ title: "My BTC address", text: address, dialogTitle: "Share BTC address" });
                  if (!ok) { const c = await copyToClipboard(address); if (c) toast.success("Address copied"); }
                }}>
              ...
              <p className="text-[11px] text-muted-foreground text-center max-w-xs">
                All addresses under{" "}
                <span className="font-mono">{BTC_DERIVATION_PATHS[BTC_DEFAULT_KIND]}/0/i</span>{" "}
